...
function needsUpdate(dest, src) {
    if (!existsSync(dest))
        return true;
    try {
        const destContent = readFileSync(dest, "utf-8");
        if (!destContent.includes(AGENT_VERSION_PATTERN))
            return false;
 ...
 */
export function ensureProjectAgentsMd(projectRoot) {
...
export function ensureAgentsInstalled() {
