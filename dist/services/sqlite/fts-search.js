...
export function searchFts(db, query, containerTag, limit) {
    ...
    try {
        const sql = containerTag === ""
           ...
           ORDER BY rank
           LIMIT ?`;
        const rows = containerTag === ""
            ? db.prepare(sql).all(ftsQuery, limit * 3)
            : db.prepare(sql).all(ftsQuery, containerTag, limit * 3);
        if (rows.length === 0)
            return [];
